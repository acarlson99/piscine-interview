...
int main(void)
{
	...
	dict = dictInit(1024000);
	for (size_t i = 0; arts[i]; ++i)
		dictInsert(dict, arts[i]->name, arts[i]);
	printf("%s\n", dictSearch(dict, arts[0]->name)->name);
	printf("%d\n", searchPrice(dict, arts[0]->name));
	printf("%s\n", dictSearch(dict, arts[3]->name)->name);
	printf("%d\n", searchPrice(dict, arts[3]->name));
